...
&= \Theta(x)  \text{, using the Master Theorem \cite{CLRS} case 1, since $a=2$, $b=2$, $f(n) = O(1) = O(n^{log_b a-1})$}\\
P(x) &= \Theta(x)\\
&= \Theta(\log n)\\
\end{aligned}
\end{equation*}
...
$cf(n) = cn^{1.8}$\\
When $c=0.9$, $af(\frac{n}{b}) \leq cf(n)$\\
...
A &= \{21, 27,33,56,65,77,79,83,88,95,101\} \text{, unchanged state of the array after no more swaps are needed}\\
\end{aligned}
\end{equation*}

\part Consider the case on an array with $n$ elements where the entire array except the last $\sqrt{n}$ elements are sorted and the last $\sqrt{n}$ elements are larger than the elements before them. The bubble sort algorithm will first scan the entire array and swap required elements in the last $\sqrt{n}$ part. Then it will scan the array up to the last but one element, then up to the last but two elements, and so on till element number $n-\sqrt{n}$. It will then stop. The number of elements scanned will be:
\begin{equation*}
\begin{aligned}
